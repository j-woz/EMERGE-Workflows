...
arguments(description         : "Run ExaEpi w/ multiple streams",
          ...
          input  params_csv   : "CSV of parameters to run");
...
(void v)
result_log_vars(string result_log_name, string envs, string kvs)
{
  ...
  @location=RL
    python_persist("import result_log",
                   ...
                   (result_log_name, envs, kvs));
  v = propagate(t);
}

result_log_write(int task_id, string result_log_name, string record)
{
  // RL: The location for the Result Log:
...
kv_array = [
               ...
               "params_csv="     + realpath_string(filename(params_csv)),
