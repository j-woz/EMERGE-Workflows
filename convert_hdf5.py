...
for field, dtype in time_varying_fields.items():
    f.create_dataset(field, (time, agent_num), dtype=dtype) #, chunks=(1, 1000000), compression="gzip", shuffle=True)
for field, dtype in constant_fields.items():
    f.create_dataset(field, (agent_num,), dtype=dtype) #, chunks=(1000000), compression="gzip", shuffle=True)
...
for i in range(block_size):
    ...
    print("loading " + data_names[dataset_idx])
    ad = ds.all_data()
    for field, dtype in time_varying_fields.items():
    ...
    ds.close()
    print("finished " + data_names[dataset_idx])
f.close()
